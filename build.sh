...
# graphics.min.js needed for tests / api / docs
STEP_GRAPHICS_FOR_CDN=true
...
if [ "${TRAVIS_BRANCH}" = "master" ] || [ ${DRY_RUN_RELEASE} ]; then
    ...
    STEP_DROP_CDN=true
    STEP_THEMES_LOCALES_GEODATA=true
...
elif [[ "${TRAVIS_BRANCH}" =~ ^([0-9]+\.[0-9]+\.[0-9]+[-rc]+[0-9]+)$ ]] || [[ "${TRAVIS_BRANCH}" =~ ^([RC-]+[0-9]+\.[0-9]+\.[0-9]+)$ ]]; then
    #TRAVIS_BRANCH =>      X.X.X-rcX it's RC build
    ...
    #TRAVIS_BRANCH =>     v8.3.0 it's a tag. Drop build
    ...
    STEP_UPLOAD_STATIC=true
    STEP_DROP_CDN=true
    # we don't need to copy all the files for develop and DVF builds
    # STEP_THEMES_LOCALES_GEODATA=true

fi
INSTALL_PACKAGE_NAME=installation-package-${VERSION}.zip
...
if [ "${STEP_GRAPHICS_FOR_CDN}" = "true" ]; then . ./bin/travis/steps/graphics_for_cdn.sh; fi
if [ "${STEP_UPLOAD_STATIC}" = "true" ]; then . ./bin/travis/steps/upload_static.sh; fi
if [ "${STEP_THEMES_LOCALES_GEODATA}" = "true" ]; then . ./bin/travis/steps/themes_locales_geodata.sh; fi
...
########################################################################################################################
#
# Upload all binaries (JS/CSS/Themes etc) on static
#
########################################################################################################################
upload_all_files
themes_locales_geodata
legacy_7x_magic
